...
export function unescapeString(stringOrTerm) {
  if (typeof stringOrTerm === "string") {
    ...
    let [integer, decimals] =
      wei.length > 18
        ? [
            wei.slice(0, wei.length - 18) || "0",
            wei.slice(wei.length - 18).replace(/0+$/, ""),
          ]
        : [
            "0",
            new Array(18 - wei.length).fill("0").join("") +
              wei.replace(/0+$/, ""),
          ];
    ...
    if (decimals.length > 5) {
      ...
      if (decimals[i + 1]) {
        ...
        decimals =
          decimals.slice(0, overflow ? i - 1 : i) +
          (overflow ? 1 : nextDigit >= 5 ? +decimals[i] + 1 : decimals[i]);
        rounded = true;
      }
    }
...
export function stringifyJson(data) {
  return JSON.stringify(data, (key, value) =>
    typeof value === "bigint"
      ? value.toString() + "n"
      : value && value.type === "BigNumber" && value.hex
