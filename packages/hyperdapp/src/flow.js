...
export async function createFlow(flowCode, { onCallFn, onCallHttp }) {
    ...
    setInputValue(type, path, value) {
      ...
      // TODO: Support more bytes/int/uint types
        ...
        if (!/^0x/.test(value) && value.length <= 64) {
          value =
            "0x" + new Array(64 - value.length).fill("0").join("") + value;
        }
    ...
    async callFn(
      ...
      return (
        ...
        returnValue.map((value, i) =>
          ...
          returnType[i] === "address" ? value.toLowerCase() : value
        )
      );
    },
  ...
  const api = {
    id: idCounter++,
    init: tryCatchProlog(async function init(
      signerAddress,
      blockNum,
      callEnv_ = {}
    ) {
      ...
      await session.promiseQuery(
        `register_addresses, current_predicate(init/0), init.`
      );
    ...
    getPrompts: tryCatchProlog(
      afterInit(async function getPrompts() {
        let prompts = [];
        ...
        if (prompts.length >= 2) {
          console.warn(
            "Multiple prompt_list answers",
            prompts.map(arrayToString)
          );
        }
        return prompts[0];
      })
    ...
    matchPrompts: tryCatchProlog(
      afterInit(async function matchPrompts(matchQuery, selectVariable) {
        const selectQuery = selectVariable
          ? `, (serialize_term(${selectVariable}, ${selectVariable}Out) -> true; ${selectVariable}Out = ${selectVariable})`
          : "";
        await session.promiseQuery(
          `get_prompts(Prompts), member(P, Prompts), prompt_exists(${matchQuery}, P)${selectQuery}.`
        );
        ...
        for await (let answer of session.promiseAnswers()) {
          ...
          answers.push(
            selectVariable
              ? {
                  [selectVariable]: answer.links[
                    selectVariable + "Out"
                  ].toJavaScript({ quoted: true }),
                }
              : {}
          );
    ...
    execute: tryCatchProlog(async function execute(actionTerms) {
      ...
      const effects = [];
      await session.promiseQuery(
        `execute_all(${arrayToString(
          actionTerms
        )}, Effects0), maplist(serialize_term, Effects0, Effects).`
      );
    ...
    handleInput: tryCatchProlog(async function handleInput(nameTerm, value) {
      ...
      await session.promiseQuery(`
        deserialize_term(${arrayToString(
          nameTerm
        )}, Name), %% Ex: [/, [/, a, b], b] -> /(/(a,b), c)
    ...
    query: tryCatchProlog(
      afterInit(async function query(query) {
        let answers = [];
        ...
        for await (let answer of session.promiseAnswers()) {
          ...
          for (let variable in answer.links) {
            result[variable] = answer.links[variable].toJavaScript({
              quoted: true,
            });
          }
          answers.push(result);
...
function arrayToString(x) {
  if (Array.isArray(x)) {
    ...
    return x.match(/^[a-z0-9'"]/i) ? x : `(${x})`; // Wrap operators in parethesis in the rare case it's needed
  }
}
